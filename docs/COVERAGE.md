...
| `tests/` unit slice | `ConfigReader`, `LayoutCss`, `TokensCss` | **100%** | `./gradlew testUnit jacocoTestUnitCoverageVerification -Denv=reference_ci` |
...
cd tests && ./gradlew testUnit jacocoTestUnitReport -Denv=reference_ci
...
Stable tag: `v0.2.1` (CI pyramid + prod pyramid green, `reference_ci` stands).
