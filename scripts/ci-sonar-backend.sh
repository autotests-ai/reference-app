#!/usr/bin/env bash
# Backend unit + JaCoCo + SonarQube upload + quality gate poll.
# Soft-skip when SONAR_TOKEN unset or host unreachable (canon: required=false until go-live).
...
export SONAR_PROJECT_KEY="${SONAR_PROJECT_KEY:-reference-app-backend}"
...
key = os.environ.get("SONAR_PROJECT_KEY", "reference-app-backend")
