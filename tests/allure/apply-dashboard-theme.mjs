 ...
 * Palette from ./pyramid-layer-colors.mjs (sync with @allure-notifications/pyramid).
 * Colors-only — no SVG reshape. Relative imports only so Allure can resolve the plugin
 * without npm install (quality-gate / gradle allureReport).
 */
...
function buildThemeJs() {
  const funnel = JSON.stringify([OTHER_LAYER, ...PYRAMID_FUNNEL_TOP_TO_BOTTOM]);
  return `/**
